"""Cafe API - REST API for cafe information."""
...
@dataclass
class Cafe(db.Model):
    """Cafe model with amenities info."""
...
def search_location():
    """Search cafes by location. Query param: loc."""
...
def add_cafe():
    """Add a new cafe. Form params: name, img_url, map_url, location, seats, has_wifi, has_sockets, has_toilet, coffee_price, can_take_calls."""
    try:
        new_cafe = Cafe(
...
def update_price(cafe_id):
    """Update cafe price. Query param: new_price."""
    ...
    return jsonify(
        {"error": {"Not Found": "Sorry a cafe with that id was not found."}}
    ), 404


@app.route("/report-closed/<int:cafe_id>", methods=["DELETE"])
def report_closed(cafe_id):
    """Delete a cafe. Requires api-key query param."""
    ...
    if api_key == "TopSecretAPIKey":
        ...
        if cafe_to_remove:
            ...
            db.session.commit()
            return jsonify(
                {"success": f"cafe with id {cafe_id} has been removed."}
            ), 200
        return jsonify(
            {"error": {"Not Found": "Sorry a cafe with that id was not found."}}
        ), 404
